...
async function api(request,env){
  ...
  if(adminSession(s)){
    ...
    if(m==='GET'&&p.match(/^\/api\/admin\/exams\/\d+\/questions$/)){const id=idNum(p.split('/')[4]);if(!id)return bad('Invalid exam');const rows=await env.DB.prepare('SELECT q.*,s.name AS skill_name FROM questions q LEFT JOIN skills s ON s.id=q.skill_id WHERE q.exam_id=? ORDER BY q.sort_order,q.id').bind(id).all();return json((rows.results||[]).map(q=>({...q,attachments:parseAttachments(q.attachments_json)})))}
