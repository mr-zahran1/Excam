...
async function api(request,env){
  ...
  if(adminSession(s)){
    ...
    if(m==='GET'&&p==='/api/admin/groups'){const rows=await env.DB.prepare(`SELECT g.*,COUNT(u.id) student_count FROM groups g LEFT JOIN users u ON u.group_id=g.id GROUP BY g.id ORDER BY g.grade,g.name`).all();return json(rows.results||[])}
