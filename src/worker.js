...
async function api(request,env){
  ...
  if(adminSession(s)){
    ...
    if(m==='GET'&&p==='/api/admin/resources'){
      const type=normalizeResourceType(url.searchParams.get('type')),grade=cleanGrade(url.searchParams.get('grade')),system=['general','azhar','all'].includes(String(url.searchParams.get('system')||'').toLowerCase())?String(url.searchParams.get('system')).toLowerCase():'';
      ...
      if(type){sql+=' AND r.type=?';params.push(type)} if(grade){sql+=' AND r.grade_level=?';params.push(grade)} if(system&&system!=='all'){sql+=` AND (r.education_system=? OR r.education_system='all')`;params.push(system)} sql+=' ORDER BY r.type,r.grade_level,r.created_at DESC,r.id DESC';
